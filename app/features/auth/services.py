...
# # from app.core.exceptions import DuplicateResourceError, AuthError, ResourceNotFoundError, BadRequestError, PermissionDeniedError
# # from app.features.auth.models import User
# # from app.features.auth.schemas import RegisterRequest, LoginRequest, PasswordChangeRequest
...
# # def create_user_account(db: Session, req: RegisterRequest, background_tasks: BackgroundTasks) -> User:
# #     """Saves user parameters as inactive, generates an OTP code, and routes it to background tasks."""
...
# #     # 1. Generate the token using the settings policy
# #     token = security.create_access_token(
# #         data={"sub": str(user.id), "role": user.role}
# #     )
    
# #     # 2. Dynamically calculate the expiration in seconds based on your .env
# #     # We multiply by 60 because ACCESS_TOKEN_EXPIRE_MINUTES is in minutes
# #     max_age_seconds = settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60
    
# #     # 3. Set the cookie using the calculated duration
# #     response.set_cookie(
# #         key="access_token",
# #         value=token,
# #         httponly=True,
...
# #         samesite="lax",
# #         max_age=max_age_seconds,  # ✅ This is where the variable is applied
# #         path="/"
# #     )
...
# #     # Security Tip: We don't throw a 404 here to prevent hackers from scanning valid user emails.
...
# #     # Overwrite old hash and clean up OTP fields
...
# #         "redirect_uri": settings.GOOGLE_REDIRECT_URI, # Points to React frontend route (e.g. http://localhost:3000/google-callback)
# #         "response_type": "code",
...
# # async def process_google_callback(code: str, db: Session) -> User:
...
# #         "redirect_uri": settings.GOOGLE_REDIRECT_URI, # Must match client configuration strictly
...
# # --- File Save Helper ---
# def save_resume_file(user_id: str, file: UploadFile) -> str:
#     """Saves a resume file securely to disk, returning the unique filename."""
#     if not os.path.exists(settings.UPLOAD_DIR):
#         os.makedirs(settings.UPLOAD_DIR)
#     # Use a clean, safe naming scheme: user_id_resume.extension
#     file_extension = os.path.splitext(file.filename)[1]
#     filename = f"{user_id}_resume{file_extension}"
...
# def delete_resume_file(filename: str) -> None:
#     """Removes a physical resume file from disk."""
#     if not filename:
#         return
#     filepath = os.path.join(settings.UPLOAD_DIR, filename)
...
#         filename = save_resume_file(new_user.id, resume)
#         new_user.resume_filename = filename
#         db.commit()
#     dispatch_registration_otp_email(background_tasks, req.email, req.name, generated_otp)
#     return new_user
...
# # --- Update update_user_profile to accept optional resume file ---
...
#     # Save/Update resume file if uploaded
#     if resume and resume.filename:
#         # Delete old file if present
#         delete_resume_file(user.resume_filename)
#         filename = save_resume_file(user.id, resume)
#         user.resume_filename = filename
#     db.commit()
#     db.refresh(user)
#     return user
# # --- Add delete_user_resume service logic ---
